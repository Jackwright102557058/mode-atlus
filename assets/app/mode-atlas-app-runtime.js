...
(function(){
  ...
  function ensureDrawer(){
    ...
    drawer.innerHTML='<div class="drawer-head"><div class="drawer-title">Profile</div><button class="drawer-close" id="profileCloseBtn" type="button">Close</button></div>'+
      ...
      '<div class="drawer-actions"><button class="drawer-action primary" id="profileSignInBtn" data-profile-sign-in type="button">Sign in</button><button class="drawer-action" id="profileSignOutBtn" data-profile-sign-out type="button" style="display:none;">Sign out</button><a class="drawer-action" href="index.html">Atlas</a><a class="drawer-action" href="kana.html">Kana Trainer</a></div>'+
...
(function(){
  ...
  function triggerFirst(){localStorage.removeItem(K.forceFirst);sessionStorage.removeItem(K.forceFirst);if(page()==='index.html')first();else{sessionStorage.setItem(K.forceFirst,'1');location.href='index.html?devFirstVisit=1'}}
  function triggerReturn(){localStorage.removeItem(K.forceReturn);sessionStorage.removeItem(K.forceReturn);if(page()==='index.html')ret();else{sessionStorage.setItem(K.forceReturn,'1');location.href='index.html?devReturn=1'}}
...
(function(){
  ...
  function injectMasteryButton(){
    document.querySelectorAll('[data-ma-mastery-open]').forEach(btn=>{ if((location.pathname.split('/').pop()||'index.html').toLowerCase()!=='kana.html') btn.remove(); });
    const page=(location.pathname.split('/').pop()||'index.html').toLowerCase();
    if(page!=='kana.html') return;
